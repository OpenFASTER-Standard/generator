...
as a `Candidate` if it resolves to exactly one node. A path that doesn't
(e.g. two structurally-identical siblings under an `xs:choice`, an
edge case this schema's real style makes unlikely but not impossible)
