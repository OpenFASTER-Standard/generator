...
- **`xsdo:documentation` is a plain RDF-native language-tagged literal** — `rdflib.Literal(text, lang="de")`/`Literal(text, lang="en")` for MiKaDiv-VIB/KaFE's real `xml:lang`-tagged documentation, and an untagged `Literal(text)` (no `lang`) for MiKaDiv-FM's own real untagged-German documentation (verified: all 410 of FM's real `xs:documentation` blocks have no `xml:lang` at all). Not a custom node shape — `rdf:langString`'s own native tagging already models this exactly. **Correction (Fix 3, final whole-branch review):** Task 4's `_extract_documentation` was only ever wired up for `xsdo:ElementDeclaration`/`xsdo:AttributeDeclaration`, leaving 175/410 real `xs:documentation` blocks (complexType/simpleType/identity-constraint-level) unextracted despite the spec's Definition of Done. It's now exported as `extraction.documentation.extract_documentation` (moved out of `extraction/declarations.py` to avoid a circular import with `extraction/simple_types.py`) and called from `extract_complex_type`, `extract_simple_type`, and `extract_identity_constraints` too.
...
- **Real, confirmed behavior this task's tests must reproduce:** `xmlschema`'s `XsdComplexType.content` returns the type's own *additional* particles only for a type using `xs:extension` — never a duplicate of the base type's own particles. `.attributes` returns the *merged* view (own + inherited); `.attributes.base_attributes` (`None` if there's no base) gives the inherited subset, used to compute the type's own-only attributes.
  **Correction, found during the final whole-branch review (2026-09-14, Fix 1):** the "own additional particles only" claim above was only verified against one real example (`DLZertifiziert`) and is false in general. When an `xs:extension` adds ONLY attributes (no own `<xs:sequence>`/`<xs:choice>`) or is genuinely empty, `xmlschema` falls back to returning the BASE type's own particle objects directly (identity-equal to `xsd_type.base_type.content.iter_model()`'s own particles) — confirmed affecting 29/53 real extension types in this corpus. `extract_complex_type` must filter these out by identity (`_base_particle_ids`, mirroring `.attributes.base_attributes`'s own-vs-inherited distinction) before minting particles, or they get duplicated under the extending type's own URI. See `PersonNatDatenType`/`PersonBasisType` (base has real element content, `Anschrift`) and `Meldeart11` (a genuinely empty extension, which would otherwise duplicate its base's entire subtree) in `tests/extraction/test_complex_types.py`.
